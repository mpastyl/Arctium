...
export CFLAGS="$CFLAGS -DFLOCKLAB=1"
...
export CFLAGS="$CFLAGS -DTX_POWER=31 -DRF_CHANNEL=26 -DCONCURRENT_TXS=5 -DNUM_ACTIVE_EPOCHS=1 -DN_TX_S=3 -DN_TX_T=3 -DN_TX_A=3 -DDUR_S_MS=6 -DDUR_T_MS=6 -DDUR_A_MS=6 -DCRYSTAL_LONGSKIP=0 -DCRYSTAL_SYNC_ACKS=1"
...
cp sndtbl.c sndtbl.c

sudo make clean
sudo rm -f crystal.sky

sudo make TARGET=sky
cp crystal.sky /media/sf_geometric_monitoring_shared/geometric-monitoring/contiki_project


#sudo make TARGET=sky MOTES=/dev/ttyUSB0 crystal.upload
#sudo make TARGET=sky MOTES=/dev/ttyUSB1 crystal.upload
#sudo make TARGET=sky MOTES=/dev/ttyUSB2 crystal.upload


#sudo make login
#sudo -E bash -c make TARGET=sky crystal.upload

#sudo -E bash -c make TARGET=sky
#sudo -E bash -c make sky-upload crystal.upload

#sudo make TARGET=sky login


#make
#sudo env CFLAGS=$CFLAGS make
#sudo env CFLAGS=$CFLAGS make
#sudo make
#make TARGET=sky
#make TARGET=sky crystal.upload
#sudo env CFLAGS=$CFLAGS make TARGET=sky crystal.upload
