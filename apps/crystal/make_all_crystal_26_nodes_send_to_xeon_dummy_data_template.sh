...
export CFLAGS="$CFLAGS -DID_BUFFER_SIZE=X"

xeon_dir="XEON_DIR_REPLACEME"
xeon_log_path="XEON_LOG_PATH_REPLACEME"
...
export CFLAGS="$CFLAGS -DTX_POWER=31 -DRF_CHANNEL=26 -DCONCURRENT_TXS=5 -DNUM_ACTIVE_EPOCHS=1 -DN_TX_S=X -DN_TX_T=X -DN_TX_A=X -DDUR_S_MS=X -DDUR_T_MS=X -DDUR_A_MS=X -DCRYSTAL_LONGSKIP=0 -DCRYSTAL_SYNC_ACKS=1"
export CFLAGS="$CFLAGS -DCRYSTAL_SINK_MAX_EMPTY_TS=X -DCRYSTAL_MAX_SILENT_TAS=X -DCRYSTAL_MAX_MISSING_ACKS=X -DCRYSTAL_SINK_MAX_NOISY_TS=6"
